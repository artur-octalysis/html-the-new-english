...
## Your workspace
Edit only `pairs/myrte-rob/`. The presenter will distribute `index.html` here after the live build. Pull the merged handout before starting. Open that file in your browser and AI editor.

## Shared protocol
Read this brief before editing. Make a branch named `pair/myrte-rob`. Keep the app self-contained. Explain the proposed change; inspect the diff; test before submitting a pull request to the original repository's main branch. Do not edit other pairs or shared infrastructure.
