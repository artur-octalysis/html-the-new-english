...
5. Open a pull request to artur-octalysis/tog-learning, targeting main. Explain the improvement and validation. If a PR already exists for this branch, update it instead. Return its link and check status. Leave merging to the presenter.`;
 ...
 $('sync-prompt').textContent=`I’m on pair/${selectedPair}. The presenter has merged the starter into the original repository’s main branch. Inspect my remotes and local changes first. Preserve my work, fetch main from artur-octalysis/tog-learning, then merge it into my current pair branch. Use upstream for a fork or origin for a direct clone. Do not reset or force-push. Confirm pairs/${selectedPair}/index.html exists and open it locally.`;
}
